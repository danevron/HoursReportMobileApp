...
let Day = React.createClass({
  ...
  _dayCircleStyle(dayType) {
    var dayCircleStyle = [styles.dayCircleFiller, this.props.customStyle.dayCircleFiller];
    switch (dayType) {
      case "workday":
        dayCircleStyle.push(styles.workdayCircle);
        break;
      case "weekend":
        dayCircleStyle.push(styles.weekendCircle);
        break;
      case "vacation":
        dayCircleStyle.push(styles.vacationDayCircle);
        break;
      case "sickness":
        dayCircleStyle.push(styles.SicknessDayCircle);
        break;
      case "holiday":
        dayCircleStyle.push(styles.holidayCircle);
        break;
      case "army":
        dayCircleStyle.push(styles.ArmyReserveDutyDayCircle);
        break;
      default:
        null;
    }
      ...
      return (
        <TouchableOpacity onPress={() => this.props.onPress(newDay)}>
          <View style={[styles.dayButton, this.props.customStyle.dayButton]}>
            <View style={this._dayCircleStyle(dayType)}>
      ...
      for (var j = 0; j < MAX_ROWS; j++) {
        if (preFiller < offset) {
          ...
          if(currentDay < daysInMonth) {
            ...
            if (!dayType && this.props.workdayDates) {
              for (var x = 0; x < this.props.workdayDates.length; x++) {
                var isWorkday = moment(this.props.workdayDates[x]).isSame(newDay, 'day') ? true : false;
                if (isWorkday) { dayType = "workday"; break; }
              }
            }
            if (!dayType && this.props.weekendDates) {
              for (var x = 0; x < this.props.weekendDates.length; x++) {
                var isWeekend = moment(this.props.weekendDates[x]).isSame(newDay, 'day') ? true : false;
                if (isWeekend) { dayType = "weekend"; break; }
              }
            }
            if (!dayType && this.props.vacationDates) {
              for (var x = 0; x < this.props.vacationDates.length; x++) {
                var isVacation = moment(this.props.vacationDates[x]).isSame(newDay, 'day') ? true : false;
                if (isVacation) { dayType = "vacation"; break; }
              }
            }
            if (!dayType && this.props.sicknessDates) {
              for (var x = 0; x < this.props.sicknessDates.length; x++) {
                var isSicknessDay = moment(this.props.sicknessDates[x]).isSame(newDay, 'day') ? true : false;
                if (isSicknessDay) { dayType = "sickness"; break; }
              }
            }
            if (!dayType && this.props.holidayDates) {
              for (var x = 0; x < this.props.holidayDates.length; x++) {
                var isHoliday = moment(this.props.holidayDates[x]).isSame(newDay, 'day') ? true : false;
                if (isHoliday) { dayType = "holiday"; break; }
              }
            }
            if (!dayType && this.props.armyDates) {
              for (var x = 0; x < this.props.armyDates.length; x++) {
                var isArmyDay = moment(this.props.armyDates[x]).isSame(newDay, 'day') ? true : false;
                if (isArmyDay) { dayType = "army"; break; }
              }
            }
            days.push((
              <Day
